...
from  core import face_detection as face_det
...
class utilities:
    ...
    def illustrate_exercise(self, example, exercise):
        ...
        if exercise != "Warm Up":
            audio.text_to_speech(instruction)
        ...
        while seconds > 0:
            ...
            speaker_thread = threading.Thread(
                target=audio.text_to_speech, args=(str(int(seconds))), kwargs={}
            )
    ...
    def repitition_counter(self, per, count, direction):
        ...
        if per == 0 and direction == 1:
            ...
            if int(count) != 0:
                print("here")
                speaker_thread = threading.Thread(
                    target=audio.text_to_speech, args=(str(int(count))), kwargs={}
                )
